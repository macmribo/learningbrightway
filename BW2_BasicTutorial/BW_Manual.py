...
# Source: [Brightway](https://2.docs.brightway.dev/intro.html#intro)
# 
# ### Brightway2 M1 install
# 
...
# 
# These rules are consistent wutg the traditional Leontief inverse of IO tables:
# 
# $ x = (I-A)^{-1}d $
# 
# Default metadata in Brightway follows ecoinvent system assumptions about biosphere flow categories:
# * *Air*, *soil*, and *water* are emissions into the natural environment.
# * *Natural resource* flows are consumtion of natural resources from the natural environment.
# 
...
# 
# Bw also provides `bw2data.backends.JSONDatabase`, which stores each dataset as a se[arate file serialized to `JSON`. This approach works well with version control  systems, as each database can be saved individually, see the [documentation](https://2.docs.brightway.dev/technical/bw2data.html#json-database).
# 
# ```{note}
# This one is a test.
# ```
